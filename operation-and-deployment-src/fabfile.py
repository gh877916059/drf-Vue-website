...
sys.path.insert(0, os.path.join(os.path.join(_ROOT_DIR, 'back-end-src'), 'APP_Inventor_case_base'))
from settings import DATABASES
_DB_USER_NAME = DATABASES['default']['USER']
_DB_PASSWORD = DATABASES['default']['PASSWORD']
_DB_NAME = DATABASES['default']['NAME']
_BACKUP_TABLES = ['cases_banner', 'cases_cases', 'cases_casescategory', 'users_userprofile']
...
def localRestoreDB():
    ...
    with lcd(backup_dir):
        ...
        makemigrationAndMigrate()
        cmd_string = 'mysql -u%s -p%s -f %s < %s' % (_DB_USER_NAME, _DB_PASSWORD, _DB_NAME, restore_file[:-7])
        local(cmd_string)
...
def remoteRestoreDB():
    ...
    with cd(_REMOTE_TMP_DIR):
        ...
        deleteAllMigrationFiles()
        makemigrationAndMigrate()
        cmd_string = 'mysql -u%s -p%s -f %s < %s' % (_DB_USER_NAME, _DB_PASSWORD, _DB_NAME, restore_file_name[:-7])
        run(cmd_string)
