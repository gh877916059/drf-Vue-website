...
class AdminSite(object):
    ...
    # 将原始视图按需增加权限检查和禁止缓存的功能再返回（最多两层修饰器）
    def admin_view(self, view, cacheable=False):
        ...
        def inner(request, *args, **kwargs):
            # 如果该试图需要权限进行访问，但此时又没有权限，则返回login试图
        ...
        # 为了使修饰器保持函数签名，需要把被装饰的函数的属性（包括名称，doc等）放到装饰的函数上
        # 要么直接在inner上增加@functools.wrap(func)装饰器；要么在return inner之前，增加一行: functools.update_wrapper(inner, func)
    ...
    def get_urls(self):
        ...
        # admin_view可以将原始视图按需增加权限检查和禁止缓存的功能再返回（最多两层修饰器）
        ...
        '''
        url()接收四个参数，分别是两个必选参数：regex、view和两个可选参数：kwargs、name
        凡是与regex匹配的URL请求都会执行到url()函数中对应的第二个参数 view 代表的视图函数中
        Django template的内建标签{% url path.to.some_view%}可以返回视图函数对应的URL（相对域名的绝对路径）
        比如 url(r^/account/$’, views.index, name=index) ，使用 {% url view.index %} 将返回 /accout/
        name用来唯一区一个视图对应多个urlconf的场景。通过name来反向获取URL
        '''
        urlpatterns = [
